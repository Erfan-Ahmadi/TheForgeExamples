...
void main()
{
	...
	vec2 tex_offset = vec2(1.0f / textureSize(sampler2D(Texture, uSampler0),0).x);
	vec3 result = texture(sampler2D(Texture, uSampler0), UV).rgb * weight[0];

	for(int i = 0; i < 5; ++i)
	{
		result += texture(sampler2D(Texture, uSampler0), UV + vec2(tex_offset.x * i, 0.0)).rgb * weight[i] * BlurStrength;
		result += texture(sampler2D(Texture, uSampler0), UV - vec2(tex_offset.x * i, 0.0)).rgb * weight[i] * BlurStrength;
	}
